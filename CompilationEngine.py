...
class Compiler():
    ...
    def compileClass(self):
        ...
        # classVarDec's
        while (self.tokenIterator.peek().stringVal.lower() in ("static", "field") and self.tokenIterator.peek().tokenType is Token.KEYWORD):
            output += f"{self.compileClassVarDec()}\n"
        # subroutine's
        while (self.tokenIterator.peek().stringVal.lower() in ("constructor", "function", "method") and self.tokenIterator.peek().tokenType is Token.KEYWORD):
    ...
    def compileClassVarDec(self):
        ...
        while (self.tokenIterator.peek().stringVal.lower() in (",") and self.tokenIterator.peek().tokenType is Token.SYMBOL):
            output += f"{self.eat(Token.SYMBOL)}\n"  # ,
    ...
    def compileSubroutineBody(self):
        ...
        output = f"{self.eat(Token.SYMBOL)}\n"  # '{'
        # subroutineBody
        while (self.tokenIterator.peek().stringVal.lower() in ("var") and self.tokenIterator.peek().tokenType is Token.KEYWORD):  # varDec*
    ...
    def compileStatements(self):
        output = ""
        while (self.tokenIterator.peek().stringVal.lower() in self.statementHandlers and self.tokenIterator.peek().tokenType is Token.KEYWORD):
    ...
    def compileReturn(self):
        ...
        if not (self.tokenIterator.peek().stringVal.lower() in (";") and self.tokenIterator.peek().tokenType is Token.SYMBOL):
            output += f"{self.compileExpression()}\n"  # expression
    ...
    def compileIf(self):
        ...
        if self.tokenIterator.peek().stringVal.lower() in ("else") and self.tokenIterator.peek().tokenType is Token.KEYWORD:
            output += f"{self.eat(Token.KEYWORD)}\n"  # 'else'
            output += f"{self.eat(Token.SYMBOL)}\n"  # '{'
            output += f"{self.compileStatements()}\n"  # statements
            output += f"{self.eat(Token.SYMBOL)}\n"  # '}'
    ...
    def compileExpressionList(self):
        ...
        output = ""
        parameters = 0
        while self.tokenIterator.peek().stringVal.lower() not in (")") or self.tokenIterator.peek().tokenType == "stringConstant":
            if (parameters > 0):
                output += f"{self.eat(Token.SYMBOL)}\n"  # ,
            output += f"{self.compileExpression()}\n"  # expression
            parameters += 1
